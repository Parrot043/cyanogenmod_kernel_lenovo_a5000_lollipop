...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.5.2 20150423"
...
static int stk3x1x_get_ps_value_only(struct stk3x1x_priv *obj, u16 ps)
{
	...
	ret = stk3x1x_master_recv(obj->client, 0x06, buf, 4);
	if(ret < 0)	
	{
		APS_ERR("%s fail, err=0x%x", __FUNCTION__, ret);
		return ret;	   
	...
	if(!(flag&STK_FLG_PSDR_MASK))
	{
		printk("%s: ps data not ready\n",__func__);
		return -1;
	}
	ret = stk3x1x_master_recv(obj->client, 0x11, buf1, 2);
	if(ret < 0)	
	{
		APS_ERR("%s fail, err=0x%x", __FUNCTION__, ret);
		return ret;	   
	}
	ps_ret = (buf1[0] << 8) | buf1[1];

	if(ps_ret >= thd_h)
		val = 0;
	else if(ps_ret < thd_l)
		val = 1;
