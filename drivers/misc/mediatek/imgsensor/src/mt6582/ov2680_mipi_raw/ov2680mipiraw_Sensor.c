...
*
*************************************************************************/
static u8 ov2680_rc = 0;
static u8 ov2680_used = 0;
    ...
    for(i = 0; i <(sizeof(sccb_writeid)/sizeof(sccb_writeid[0])); i++)
    {
        ...
        *sensorID=((OV2680_read_cmos_sensor(0x300A) << 8) | OV2680_read_cmos_sensor(0x300B));	
        ...
        if(OV2680MIPI_SENSOR_ID == *sensorID)
        {
					...
					dev->device_type = "CCM";
					dev->device_module = "huaquan";
					dev->device_vendor	 ="huaquan";	  
					dev->device_ic	 = "OV2680";
...
{
#ifdef OV2680_DRIVER_TRACE
    SENSORDB("OV2680GetInfo\A3\ACFeatureId:%d\n",ScenarioId);
#endif
...
{
#ifdef OV2680_DRIVER_TRACE
    SENSORDB("OV2680Control\A3\ACScenarioId:%d\n",ScenarioId);
#endif	
...
#ifdef OV2680_DRIVER_TRACE
    SENSORDB("OV2680SetVideoMode\A3\ACu2FrameRate:%d\n",u2FrameRate);
#endif	
...
#ifdef OV2680_DRIVER_TRACE
    //SENSORDB("OV2680FeatureControl\A3\ACFeatureId:%d\n",FeatureId); 
#endif		
    switch (FeatureId)
    {
